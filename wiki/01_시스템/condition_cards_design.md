...
1.  **정신이상 상태 (Insanity / Mental Damage)**:
    ...
    *   **환각 (Hallucinations)**: 허상을 봄. (응징 격발 시: 환각에 굴복하여 자신의 체력/스태미나를 깎거나 몬스터와 조우 처리)
...
- **제거 방법**: 폐양조장의 마녀에게 `진정제 식물`을 구매해 복용하거나 성당 본당의 **신부 (Priest)**에게 `정신 정화`를 받습니다.

#### B. 편집증 (Paranoia)
- **효과**: 
  - 플레이어: NPA와의 단서 교환(`request_clue`) 시 요구되는 신뢰도(Trust) 장벽이 2배로 상승합니다.
  - NPA: 플레이어와의 대화를 거부하며, 밤이 되면 구석에 숨어 조우가 불가능해집니다.
- **제거 방법**: 성당 본당의 신부와 장기 심리 상담을 받거나 여관 로비에서 술을 대량 구매하여 NPA들과 나누어 마십니다.

#### C. 기억상실 (Amnesia)
- **효과**:
  - 매일 자정 단계(Midnight Phase)가 될 때마다 소지하고 있는 **단서 토큰(Clue Tokens)을 무조건 1개씩 망각하여 소멸**시킵니다.
- **제거 방법**: 여관 로비에서 2일간 아무것도 하지 않고 순수하게 수면을 취해 뇌 세포를 안정시킵니다.

#### D. 우울증 (Melancholy)
- **효과**:
  - 여관 로비나 성당 본당에서 휴식 취하기 행동 시 획득하는 HP 및 스태미나 회복 효율이 반감(50%)됩니다.
- **제거 방법**: 여관 로비에서 NPA들과 함께 골드를 소비하여 파티를 벌이거나 특정 돌발 긍정 이벤트를 마주합니다.
...
- **제거 방법**: 성당 본당에서 기도를 통해 치료를 받거나 `치료 약초(Healing Herb)`를 사용해 자가 지혈합니다.

#### B. 다리 골절 (Fractured Leg)
- **효과**:
  - 도시 내 장소 이동 시 기본으로 소모되는 행동력(AP) 또는 이동력(Movement) 수치가 **2배로 증가**합니다. (예: 1 AP 소모 골목길 이동이 2 AP 소모로 변경)
- **제거 방법**: 성당 본당의 신부에게서 골절 접합 치료를 받거나 여관 로비에서 3일간 요양합니다.
...
- **제거 방법**: 성당 본당의 성수대에서 `성수(Holy Water)`를 받아 전신을 정화합니다.
...
### 3.2 정적 NPC 성당 본당 연동 코드 (GDScript 의사코드)
