...
* 잠시 이야기한다
  ~ spend_ap(1)
  짧은 대화였지만, 행동력을 소모해 시간은 흘렀어요. # speaker=엘레나
...
| `set_metric(key, value)` | `~ set_metric("stamina", 50)` | 수치형 변수 설정 |
| `change_metric(key, amount)` | `~ change_metric("stamina", 10)` | 수치 증감 (+/-) |
| `spend_ap(amount)` | `~ spend_ap(3)` | 행동력(AP) 소모 |
| `spend_movement(amount)` | `~ spend_movement(1)` | 이동력 소모 |
...
```ink
* { stamina > 50 } 자신감 있게 대답한다.
  오? 당신 자신감 있네요. # speaker=모리건

* { stamina <= 50 } 겁먹은 채 대답한다.
  ...뭔가 위험한 건가요? # speaker=모리건
```

- `{ 조건 }`을 선택지 `*` 바로 뒤에 붙입니다.
- `Flags.` 접두사는 **생략**합니다. `Flags.stamina > 50` → `stamina > 50`
...
> [!NOTE]
> 아래 예시는 행동력(AP) 시스템을 사용하는 표준 작성 방식입니다.
