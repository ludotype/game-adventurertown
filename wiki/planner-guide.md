...
| condition | 예시 | 설명 |
|-----------|------|------|
| `flag_eq` | `{ "flag_eq": ["guild_member", true] }` | 플래그 값이 같은지 확인합니다. |
...
| 전역 값 | `global.{metric}` | `global.guild_reputation` |
...
├── scripts/state/        ← 날짜/시간, MetricStore 등 런타임 상태 코드 (직접 수정 불필요)
