# Adventure Town — 아키텍처 Battle Plan

> 본 문서는 Adventure Town 프로젝트의 **장기 설계 기준**이다.
> 모든 시스템 추가·수정 결정은 이 문서의 원칙(§0)을 최우선으로 따른다.
>
> 작성 시점: Phase 0 (장소·이동·행동·NPC 추첨 기본 골격 완료)
...
│  │ DialogueRegistry           │  │ NPCRelations                  │  │
│  │ EventRegistry              │  │ QuestLog                      │  │
│  └────────────────────────────┘  └───────────────────────────────┘  │
│                                                                     │
│  ┌─ Runtime Engines (logic) ─────────────────────────────────────┐  │
│  │ ActionRunner   ConditionEvaluator   EffectApplier   EventBus  │  │
...
```json
{
  "place_id": "guild_office",
  "display_name": "길드 사무소",
  "background_path": "res://assets/bg/guild_office.png",
  "bgm": "town_day",
  "empty_weight": 0,
  "tags": ["indoor", "service", "guild"],
  "connections": ["town_square"],
  "on_enter": [
    { "type": "set_flag", "key": "visited_guild_office", "value": true }
  ]
}
```

### 2.2 NPC — `data/npcs/{npc_id}.json`

> 기존 `data/npc_schedules/`를 `data/npcs/`로 통합한다 (Phase 3에서 마이그레이션).
...
```json
{
  ...
  "tags": ["guild_receptionist", "human"],
  "schedules": [
    { "place_id": "guild_office", "weight": 10,
...
```json
{
  "item_id": "guild_card",
  "display_name": "길드 신분증",
  "icon_path": "res://assets/items/guild_card.png",
  "stackable": false,
  "tags": ["key_item"]
}
```
...
```json
{
  "event_id": "first_morning_in_inn",
  "trigger": {
    "on": "place_entered",
    "place_id": "inn_room",
    "when": { "flag_eq": ["intro_seen", false] }
  },
  "actions": [
    { "type": "set_flag", "key": "intro_seen", "value": true },
    { "type": "dialogue", "dialogue_id": "intro_morning" }
  ],
  "consume": true
}
```
...
| `advance_time` | `time_units=1` | 기본 시간 단위만큼 시간 진행 |
| `advance_minutes` | `minutes` | 예외적으로 정확한 분 단위 시간 진행 |
...
| `time_block` | block 또는 block[] |
| `time_in` | time block[] (기존 호환 별칭) |
...
| 범위 | 형식 | 예시 |
|---|---|---|
| NPC | `npc.{npc_id}.{metric}` | `npc.elena.affection`, `npc.luise.mood` |
...
- `relation_gte`, `npc_mood_gte` 같은 전용 문법은 필요성이 반복적으로 증명된 뒤 편의 별칭으로만 추가한다.
...
**검수 기준:** 길드 등록 시퀀스(여러 액션 합성 + 플래그 분기)가 JSON만으로 동작한다.

### Phase 2 — TimeSystem (진행 중)

**목표:** 시간이 흐른다. NPC 스케줄이 진짜로 시간을 따른다.

- `TimeSystem` autoload — `day`, `hour`, `minute`, `minutes_per_time_unit`, signal `time_advanced`
- 액션 추가: `advance_time`, `advance_minutes`, `sleep_until_next_day`
- 조건 추가: `time_block`, `time_in`, `hour_range`, `time_range`, `day_eq`, `day_gte`
- `NPCSpawner`가 하드코딩된 `"morning"` 대신 `TimeSystem.current_time_of_day` 참조

**현재 완료:** 24시간제 `TimeSystem` autoload 추가, `time_units` 기반 `advance_time`, `advance_minutes`, `sleep_until_next_day`, 시간 블록/정밀 시간 조건 구현, 장소 UI에 `Day N HH:MM` 표시.

**검수 기준:** "잠자고 하루 넘기기" 행동이 시간을 진행시키고, NPC 스케줄이 실제로 시간 따라 바뀐다.
...
**검수 기준:** 엘레나 interaction에서 `npc.elena.affection`, `npc.elena.mood` 값을 JSON 액션으로 변경하고, metric 조건에 따라 다른 이벤트가 선택된다.
...
**검수 기준:** 첫 아침 인트로 이벤트가 `place_entered: inn_room` + `intro_seen=false` 조건으로 자동 발화하고, 두 번 발화하지 않는다.
...
### 시나리오 A — "잡화상 + 상인 잭 + 시간대별 상품"

```
1. data/places/general_store.json 작성
   - connections: ["town_square"]
   - actions: [{ "id":"talk_jack", "label":"잭과 대화", "type":"dialogue", "dialogue_id":"jack_default" }]

2. data/npcs/jack.json 작성
   - schedules: [{ "place_id":"general_store", "weight":10,
                   "conditions": {"time_of_day":["morning","afternoon","evening"]} }]
   - default_dialogue: "jack_default"

3. data/dialogues/jack_default.dialogue 작성
   - 시간대별 분기 (DialogueManager 표준 문법)
   - 안에서 do ActionRunner.run({"type":"sequence","actions":[
       {"type":"give_item","item_id":"health_potion","qty":1},
       {"type":"take_item","item_id":"gold","qty":50}
     ]})

4. data/items/health_potion.json, gold.json 작성 (없으면)
```
...
```json
// data/events/elena_first_meet.json
{
  "event_id": "elena_first_meet",
  "trigger": {
    "on": "place_entered",
    "place_id": "town_square",
    "when": {
      "all_of": [
        { "flag_eq": ["met_elena", false] },
        { "time_in": ["morning"] }
      ]
    }
  },
  "actions": [
    { "type": "set_flag", "key": "met_elena", "value": true },
    { "type": "dialogue", "dialogue_id": "elena_meet_cute" }
  ],
  "consume": true
}
```
...
1. **합성 어휘 (sequence/if/choice/random/call)** — 단순 어휘만으로 RPG Maker급 이벤트 표현 가능.
...
4. **tags 시스템** — 단순 ID 매칭을 넘어 "indoor 장소에서만", "guild 태그 NPC만" 같은 패턴 매칭 가능 (Phase 후반 확장).
5. **NPC schedule + condition** — NPC가 시간/플래그/날짜에 따라 다른 곳에 있고, 다른 대사를 한다.

Princess Maker · Stardew Valley · Fire Emblem 지원관 · Persona의 페르소나 시간표 — 이 정도 깊이까지 본 구조로 도달 가능하다. 한계는 어휘를 얼마나 늘리느냐에 달려 있다.
