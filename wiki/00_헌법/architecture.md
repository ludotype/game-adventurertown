...
```json
{
  "place_id": "curio_shop",
  "display_name": "골동품 상점",
  "description": "기이한 고대 유물과 진귀한 신비 용품이 진열된 수상쩍은 상점.",
  "descriptions": {
    "morning": "아침 햇살 속에서 먼지 쌓인 유물들이 이상한 빛을 낸다.",
    "night": "가게 안은 어둑어둑하고, 촛불만이 희미하게 흔들린다."
  },
  "sub_npcs": [
    { "npc_id": "curio_dealer", "display_name": "골동품 상인", "description": "깊은 두건 아래에서 이상한 물건들을 판매하는 상인." }
  ],
  "background_path": "res://assets/bg/curio_shop.png",
  "bgm": "town_day",
  "empty_weight": 6,
  "tags": ["indoor", "shop"],
  "connections": ["main_avenue"],
  "on_enter": [
    { "type": "set_flag", "key": "visited_curio_shop", "value": true }
  ]
}
```
...
```json
{
  ...
  "schedules": [
    { "place_id": "lobby", "weight": 10,
      "conditions": { "time_of_day": ["morning", "afternoon", "evening", "night"] } }
  ]
}
```
...
```json
{
  "event_id": "luise_first_evening_lobby",
  "trigger": {
    "on": "place_entered",
    "place_id": "lobby",
...
| `change_relation` | `npc_id`, `delta` | 추후 편의 별칭. 내부적으로 `npc.{npc_id}.trust` metric 사용 |
...
| 범위 | 형식 | 예시 |
|---|---|---|
| NPC | `npc.{npc_id}.{metric}` | `npc.luise.trust`, `npc.luise.mood` |
| 장소 | `place.{place_id}.{metric}` | `place.lobby.clean_count` |
| 플레이어 | `player.{metric}` | `player.music`, `player.physique` |
...
- 데이터: `data/places/lobby.json`, `main_avenue.json`
...
- `MetricStore` 기반으로 trust, mood, clean_count 같은 자잘한 상태를 관리한다.
- 액션 `set_metric`, `change_metric`, 조건 `metric_gte` 등으로 interaction 분기를 만든다.

**현재 완료:** 기존 `data/npc_schedules/` 구조를 유지한 채 `default_dialogue`, `dialogue_id` 전달, NPC 초상화 클릭 대화 실행, `data/dialogues/` 샘플 대화 추가, `InteractionRegistry` 및 `data/interactions/common`, `place/{place_id}`, `char/luise` 샘플 추가, `places/*.json`의 과도기 `actions` 제거, `MetricStore`와 metric 조건/액션 추가.

**검수 기준:** 루이제 interaction에서 `npc.luise.trust`, `npc.luise.mood` 값을 JSON 액션으로 변경하고, metric 조건에 따라 다른 이벤트가 선택된다.
...
**검수 기준:** 첫 만남 이벤트가 `place_entered: lobby` + `met_luise=false` 조건으로 자동 발화하고, 두 번 발화하지 않는다.
...
### 시나리오 A — "골동품 상점 + 골동품 상인 + 시간대별 상호작용"

```
1. data/places/curio_shop.json 작성
   - connections: ["main_avenue"]

2. data/npc_schedules/curio_dealer.json 작성
   - schedules: [{ "place_id":"curio_shop", "weight":10,
                   "conditions": {"time_of_day":["morning","afternoon"]} }]
   - default_dialogue: "curio_dealer_default"

3. data/dialogues/curio_dealer_default.dialogue 작성
   - 시간대별 분기 (DialogueManager 표준 문법)
   - 안에서 do ActionRunner.run({"type":"sequence","actions":[
       {"type":"give_item","item_id":"holy_symbol","qty":1},
       {"type":"change_metric","key":"npc.curio_dealer.trust","amount":1}
     ]})
...
| R7 | 시나리오 작업량 (NPC 대사) | 핵심 NPC 집중으로 범위 제한. 단계별 이벤트를 점진적 추가. |
...
- Dialogue Manager: [[dialogue_manager_guide]]
